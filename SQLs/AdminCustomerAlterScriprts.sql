...
BEGIN TRY
    ...
    IF NOT EXISTS (SELECT 1 FROM dbo.CustomerRole WHERE SystemName = N'ViewNopStationPlugin' OR Name = N'View NopStation Plugin')
    BEGIN
        ...
        VALUES (
            N'View NopStation Plugin', N'ViewNopStationPlugin', 0, 0, 1, 1, -- IsSystemRole = 1
            0, 0, 0, 0
        );
    ...
    ELSE
    BEGIN
        PRINT 'ViewNopStationPlugin role already exists. Ensuring correct SystemName, Active status, and IsSystemRole...';
        UPDATE dbo.CustomerRole
        SET SystemName = N'ViewNopStationPlugin',
            Active = 1,
            IsSystemRole = 1
        WHERE SystemName = N'ViewNopStationPlugin' OR Name = N'View NopStation Plugin';
        PRINT 'ViewNopStationPlugin role updated successfully.';
    END
    ...
    -- -----------------------------------------------------------------------------
    -- 3.1 Ensure permission mappings for ViewNopStationPlugin role
    -- -----------------------------------------------------------------------------
    PRINT 'Configuring permission mappings for ViewNopStationPlugin role...';
    
    -- Remove all existing permission record mappings for the ViewNopStationPlugin role
    DELETE FROM dbo.PermissionRecord_Role_Mapping WHERE CustomerRole_Id = @ViewPluginRoleId;
    PRINT 'Removed existing permission mappings for ViewNopStationPlugin role.';

    -- Insert new permission mappings based on the specified criteria
    INSERT INTO dbo.PermissionRecord_Role_Mapping (PermissionRecord_Id, CustomerRole_Id)
    SELECT pr.[Id], @ViewPluginRoleId
    FROM dbo.[PermissionRecord] pr 
    WHERE ((pr.[Name] LIKE '%Nop%' OR pr.[SystemName] LIKE '%Nop%') 
           AND pr.SystemName NOT IN (
               N'ManageNopStationCoreLicense',
               N'ManageNopStationCoreConfiguration',
               N'ManageNopStationFeatures'
           )) 
       OR pr.SystemName IN (
           N'Security.AccessAdminPanel',
           N'PublicStore.PublicStoreAllowNavigation',
           N'PublicStore.AccessClosedStore'
       );

    PRINT 'Mapped new permissions to ViewNopStationPlugin role.';
    ...
    WHERE Email NOT IN (SELECT Email FROM @ExcludedEmails)
